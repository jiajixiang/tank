...
class Avatar(KBEngine.Entity,
	...
	def Fire(self, exposed, value):
		DEBUG_MSG(" Avatar Fire self.id = %i exposed = %i damage = %i" % (self.id, exposed, value))
		"""
		defined.
		玩家跳跃 我们广播这个行为
		"""
		...
		self.otherClients.OnFire(value)

	def takeDamage(self, receiverID, damage):
		DEBUG_MSG(" Avatar takeDamage self.id = %i receiverID = %i damage = %i" % (self.id, receiverID, damage))
		if receiverID == self.id:
			Combat.recvDamage(self, self.id, 1, 0, damage)
		target = KBEngine.entities.get(receiverID)
		if target is None:
			ERROR_MSG("Spell::spellTarget( %i ):receiverID = %i not found" % (self.id, receiverID))
			return
